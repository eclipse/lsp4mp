 ...
 */
//@RequestScoped
public class CircuitBreakerClientForValidationDelay {

    @CircuitBreaker(failureRatio = -1) // error here 
    public Connection invalidA() {
        return null;
    }
    
    @CircuitBreaker(failureRatio = 2) // error here 
    public Connection invalidB() {
        return null;
    }
    
    @CircuitBreaker(failureRatio = 0)
